...
metadata {
	definition (
		...
		vid: "78ca5190-9667-39f3-97b0-29393936bb65"
	) {
		capability "Actuator"
		capability "Switch"
		capability "Refresh"
		...
		fingerprint mfr: "027A", prod: "0104", model: "0202", deviceJoinName: "Zooz ZEN52 Double Relay"
	}
	...
	relayEndpoints.each { endpoint ->
		if (!findChildByEndpoint(endpoint)) {
			...
			try {
				...
				child = createChildDevice(endpoint, dni, "SmartThings", "Child Switch")
			}
...
def createChildDevice(int endpoint, String dni, String dthNamespace, String dthName) {
	return addChildDevice(
		...
		[
			completedSetup: true,
			label: "Zooz ZEN52 Relay ${endpoint}",
			isComponent: false
		]
	)
}
...
void executeOnOffCmds(int value, endpoint) {
	...
	// Workaround for automatic reports being extremely unreliable.
...
String switchBinarySetCmd(int value, int endpoint) {
...
void sendSwitchEvent(int rawValue, int endpoint) {
	...
	if (endpoint == mainEndpoint) {
		...
		if (child) {
			logDebug("${child.displayName} switch is ${value}")
