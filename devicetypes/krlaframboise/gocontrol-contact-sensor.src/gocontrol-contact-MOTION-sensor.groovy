/**
 *  GoControl Contact (MOTION) Sensor v1.7
 *  (WADWAZ-1)
 *
 *  Author: 
...
metadata {
	definition (
		name: "GoControl Contact (MOTION) Sensor", 
		...
		attribute "internalContact", "enum", ["active", "inactive"]
		attribute "externalContact", "enum", ["active", "inactive"]
	...
	tiles(scale: 2) {
		...
		main("motion")
		details(["motion", "battery", "tampering", "refresh"])
	}
}
...
private handleContactEvent(alarmLevel, attr) {
	def result = []
	def val = (alarmLevel == 0xFF) ? "active" : "inactive"
	...
	if (mainVal) {
		result << createEvent(getEventMap("motion", mainVal))
	}
	return result
}
...
private getMainContactVal(activeAttr, activeVal, otherVal) {
	...
	switch (settings?.mainContactBehavior) {
		...
		case "Both Contacts Closed":
			mainVal = (activeVal == "inactive" && otherVal == "inactive") ? "inactive" : "active"
			break
		case "Both Contacts Open":
			mainVal = (activeVal == "active" && otherVal == "active") ? "active" : "inactive"
			break
		default:
			mainVal = activeVal
	}	
