/**
 *  GoControl Contact (WATER) Sensor v1.7
 *  (WADWAZ-1)
 *
 *  Author: 
...
metadata {
	definition (
		name: "GoControl Contact (WATER) Sensor", 
		...
		attribute "internalContact", "enum", ["wet", "dry"]
		attribute "externalContact", "enum", ["wet", "dry"]
		attribute "lastCheckin", "number"

		// deviceId: "0x2001", inClusters: "0x71,0x85,0x80,0x72,0x30,0x86,0x84"			
		// type:"2001", cc:"71,85,80,72,30,86,84"
		// mfr:"014F", prod:"2001", model:"0102"
	}
	...
	tiles(scale: 2) {
		...
		main("water")
		details(["water", "battery", "tampering", "refresh"])
	}
}
...
def configure() {	
	...
	if (!device.currentValue("water")) {
		sendEvent(name: "water", value: "wet", isStateChange: true, displayed: false)
	}
...
private handleContactEvent(alarmLevel, attr) {
	def result = []
	def val = (alarmLevel == 0xFF) ? "wet" : "dry"
	...
	if (mainVal) {
		result << createEvent(getEventMap("water", mainVal))
	}
	return result
}
...
private getMainContactVal(activeAttr, activeVal, otherVal) {
	...
	switch (settings?.mainContactBehavior) {
		...
		case "Both Contacts Closed":
			mainVal = (activeVal == "dry" && otherVal == "dry") ? "dry" : "wet"
			break
		case "Both Contacts Open":
			mainVal = (activeVal == "wet" && otherVal == "wet") ? "wet" : "dry"
			break
		default:
			mainVal = activeVal
	}	
