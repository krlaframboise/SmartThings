/**
 *  BeSense Contact Sensor Zwave Plus v1.0
 *  (Model: IM20)
 *
 *  Author: 
...
metadata {
	definition (
		name: "BeSense Contact Sensor Zwave Plus", 
		...
		fingerprint mfr:"0214", prod:"0002", model:"0001", deviceJoinName: "BeSense Contact Sensor Zwave Plus"
	}
	...
	tiles(scale: 2) {
		...
		main(["contact"])
		details(["contact", "battery", "configStatus", "pendingChanges", "refresh", "firmwareVersion"])
	}
...
def zwaveEvent(physicalgraph.zwave.commands.notificationv3.NotificationReport cmd) {
	...
	if (cmd.notificationType == 7) {
		// Ignoring Tamper events because they're being handled from the Sensor Binary Reports.		
	}
	else if (cmd.notificationType == 6) {
		// Ignoring Motion events because they're being handled from the Sensor Binary Reports.
	}
	else {
...
private sendContactEvent(value) {
	logDebug "contact ${value}"
	sendEvent(getEventMap("contact", value))
}
