/**
 *  Aeon Labs Multifunction Doorbell v 1.6a
 *
 *  Capabilities:
 ...
 *
 *	1.6 (02/09/2016)
 *		-	Added preference for non secure.
 *
 *	1.5 (02/08/2016)
 *		-	Fixed fingerprint
...
metadata {
	...
	preferences {
		...
		input "useSecureCommands", "bool", title: "Secure Commands?", defaultValue: true, displayDuringSetup: true, required: false
...
private secureCommand(physicalgraph.zwave.Command cmd) {
	if (state.useSecureCommands) {
		writeToDebugLog("secureCommand")
		zwave.securityV1.securityMessageEncapsulation().encapsulate(cmd).format()
	} else {
		writeToDebugLog("nonSecureCommand")
		cmd.format()
	}
}
